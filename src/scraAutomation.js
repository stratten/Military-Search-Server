...
async function runScraAutomation({
  ...
  try {
    ...
    if (await page.$('input#username')) {
      console.log('Login form detected (type 1), logging in...');
      try {
    ...
    // Check for alternative login form seen in the screenshot
    else if (await page.$('input[name="Password"]') || await page.$('input[name="username"]')) {
      console.log('Login form detected (type 2), logging in...');
      try {
        // Take a screenshot before login attempt
        await page.screenshot({ path: path.join(runFolder, 'screenshot_before_login.png') });
        
        // Try different selectors for username/password
        const usernameSelectors = ['input[name="username"]', '#Username', 'input[name="Username"]'];
